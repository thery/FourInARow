...
Definition htop_eval67 w b ht :=
  let: PRes sc _ _ := 
    alpha_beta (1 + nheight * nwidth) 0 w b loss win zero ht in sc.

Lemma htop_eval67_equiv w b ht : 
  htop_eval 6 7 w b ht  = htop_eval67 w b ht.
Proof. by []. Qed.

Lemma htopeval67_correct w b ht : 
 valid_posb 6 7 w b -> valid_htable 6 7 ht -> valid_eval 6 7 w b (htop_eval67 w b ht).
Proof.
move=> Hv Ht.
rewrite -htop_eval67_equiv.
apply: htopeval_correct; [done|done|done|done| |done|done].
rewrite -to_nat_lsl_one; last by [].
by apply: nltbP.
Qed.

Definition top_eval67 w b := htop_eval67 w b (make_hash tt) .

Lemma topeval67_correct w b : valid_posb 6 7 w b -> valid_eval 6 7 w b (top_eval67 w b).
Proof.
move=> Hv.
rewrite -[top_eval67 _ _]htop_eval67_equiv.
