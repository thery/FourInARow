...
Require Import FourInARow74.

Set Implicit Arguments.
Unset Strict Implicit.
Unset Printing Implicit Defensive.

Definition htop_eval74 w b ht :=
  let: PRes sc _ _ := 
    alpha_beta (1 + nheight * nwidth) 0 w b loss win zero ht in sc.

Lemma htop_eval74_equiv w b ht : 
  htop_eval 7 4 w b ht  = htop_eval74 w b ht.
Proof. by []. Qed.

Lemma htopeval74_correct w b ht : 
 valid_posb 7 4 w b -> valid_htable 7 4 ht -> valid_eval 7 4 w b (htop_eval74 w b ht).
Proof.
move=> Hv Ht.
rewrite -htop_eval74_equiv.
apply: htopeval_correct; [done|done|done|done| |done|done].
rewrite -to_nat_lsl_one; last by [].
by apply: nltbP.
Qed.

Definition top_eval74 w b := htop_eval74 w b (make_hash tt) .

Lemma topeval74_correct w b : valid_posb 7 4 w b -> valid_eval 7 4 w b (top_eval74 w b).
Proof.
move=> Hv.
rewrite -[top_eval74 _ _]htop_eval74_equiv.
