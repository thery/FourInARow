...
case: ifP => Hif1.
  ...
  apply/existsP/existsP=> [] [i1 /existsP[j1 /and3P[H1 H2 H3]]].
...
Lemma fms_draw_rect_corect w b res i cols : 
  wf_state (w lor b) -> 
  (size cols + i = nwidth)%N ->
  (forall j, j < size cols -> 
      nth 0 cols j = lsl first_column (of_nat (j + i) * horizontal)) ->
...
case: ifP => Hif1.
  ...
  apply/forallP/forallP => /= H i1; apply/forallP => /= j1; apply/implyP => H1;
      last first.
    have /forallP/(_ j1)/implyP->// := H i1.
    by apply: ltnW.
  case: ltngtP H1 => // [iLi1 _|iEi1 _].
    by have /forallP/(_ j1)/implyP/(_ iLi1) := H i1.
  have j1Lw : j1 < nwB by apply: ltn_trans _ nheightLwB.
  apply/negP => H1.
  rewrite -get_border_correct // -iEi1 in H1.
  suff : cell (get_border w b land c) i j1.
    by have /is_zero_spec-> := Hif1; rewrite cell_0.
  rewrite cell_land H1 Hc /cell bit_lsl ifN.
    rewrite add_comm addK first_column_spec.
...
Lemma fmt_forced_rect_corect w b res i k cols : 
  wf_state (w lor b) -> 
  (size cols + i = nwidth)%N ->
  (forall j, j < size cols -> 
      nth 0 cols j = lsl first_column (of_nat (j + i) * horizontal)) ->
...
have cmE : cwin (mk_move w i (to_nat k1 %% nhorizontal)) = 
  ...
  rewrite bit_cell get_border_correct //; last 2 first.
  - by rewrite -iE'.
  - by move: bck2; rewrite Hc => /(bit_lsl_first_column_mod_lt _ _ iLw).
  move=> /cmoveE k2mE.
  move: bE.
  rewrite bit_cell get_border_correct //; last by rewrite -iE.
  move=> /cmoveE kmE.
...
rewrite IH'; case: eqP => //= _.
apply/forallP/forallP => /= H i1; apply/forallP => /= j1;
    apply/implyP => H1; apply/implyP => H2; last first.
  by have /forallP/(_ j1) := H i1; rewrite ltnW //= H2.
move: H1; case: ltngtP => // iEi2 _.
  by have /forallP/(_ j1) := H i1; rewrite iEi2 //= H2.
by rewrite (cmoveE _ _ _ H2) -iEi2 -(cmoveE _ _ _ cM) cmE HE.
Qed.

Lemma fms_forced_rect_corect w b res i k cols : 
  wf_state (w lor b) -> 
  (size cols + i = nwidth)%N ->
  (forall j, j < size cols -> 
      nth 0 cols j = lsl first_column (of_nat (j + i) * horizontal)) ->
...
case: ifP => Hif1.
  ...
  have Hcm j2 : j2 < nheight -> ~~ cmove (w lor b) i j2.
    move=> j2Lh.
    rewrite -get_border_correct //=; apply/negP=> H1.
    ...
    apply: leq_trans (_ : nwidth * nhorizontal <= _) => //.
    by rewrite leq_mul2r iLw.
  move=> [H1 H2]; split => [i1 j1|//].
  case: ltngtP => //= iEi2 _ H3; first by apply: H1.
...
case: ifP => HE //.
case: ifP => HE1.
  rewrite (fmt_forced_rect_corect _ _ _ i.+1) //.
  ...
  split => [i1 j1|].
    ...
    rewrite -iEi1 in cM1 *.
    by rewrite (cmoveE _ _ _ cM1) -(cmoveE _ _ _ cM) cmE HE.
  exists i; exists (to_nat k1 %% nhorizontal); split => //.
    rewrite iE -gE gbcL; congr lsl.
    by rewrite [LHS](of_nat_int_add_mod k1 horizontal).
  by rewrite cmE1 HE1.
move=> /IH' [H1 H2]; split => // i1 j1.
case: ltngtP => // [iLi1|iEi1] _; first by apply: H1.
move=> cM1.
by rewrite (cmoveE _ _ _ cM1) -iEi1 -(cmoveE _ _ _ cM) cmE HE.
Qed.
  ...
  fms w b (get_border w b) columns [::] == Forced k -> 
    ...
    (exists i1, exists j1,  
      [/\ 
         ...
         cmove (w lor b) i1 j1 & cwin (mk_move b i1 j1)]).
Proof.
move=> Hwf /(fms_forced_rect_corect _ _ _ 0) // /(_ Hwf) [].
- by rewrite addn0 columns_size.
- by move=> j jLc; rewrite addn0 columns_val.
by move=> H1 H2; split => // i1 j1; apply: H1.
Qed.
...
move=> /IH H1; apply: subset_trans H1.
apply/subsetP =>  /= [] [x y]; rewrite mem_insert_fmove => ->.
by rewrite orbT.
Qed.

Lemma fms_moves_rect_corect w b res i l cols : 
  wf_state (w lor b) -> 
  (size cols + i = nwidth)%N ->
  (forall j, j < size cols -> 
      nth 0 cols j = lsl first_column (of_nat (j + i) * horizontal)) ->
...
Proof.
move=> Hwf.
elim: cols i res => /= [i res|c cols IH i res Hsi Hf Hin].
  rewrite add0n => -> _.
  case: res => //= [] [m v] l1 Hin /eqP[<-]; split => //=.
  - by move=> i1 j1 H1 /and3P[]; rewrite ltnNge H1.
  - by move=> i1 j1 H1 /and3P[]; rewrite ltnNge H1.
  by move=> i1 j1 H1 /and3P[]; rewrite ltnNge H1.
have iLw : i < nwidth by rewrite -Hsi addSn ltnS leq_addl.
have iLwB : i < nwB by apply: ltn_trans nwidthLwB.
...
case: ifP => Hif1.
  ...
  have Hcm j2 : j2 < nheight -> ~~ cmove (w lor b) i j2.
    ...
    apply: leq_trans (_ : nwidth * nhorizontal <= _) => //.
    by rewrite leq_mul2r iLw.
  move=> /(_ Hin) [H1 H2 H3 H4 H5]; split => //.
  - move=> i1 j1; case: ltngtP => // [i1Lj1|<-] _ HH; first by apply: H1.
    have j1Lh : j1 < nheight by case/and3P : HH.
    by have /negP[] := (Hcm j1 j1Lh).
  - move=> i1 j1; case: ltngtP => // [i1Lj1|<-] _ HH; first by apply: H2.
    have j1Lh : j1 < nheight by case/and3P : HH.
    by have /negP[] := (Hcm j1 j1Lh).
  move=> i1 j1; case: ltngtP => // [i1Lj1|<-] _ HH; first by apply: H5.
  have j1Lh : j1 < nheight by case/and3P : HH.
  by have /negP[] := (Hcm j1 j1Lh).
have /negP/negP := Hif1; rewrite is_zeroNP =>/existsP[/= k1 k1E].
have : bit c k1 by move: k1E; rewrite land_spec; case: bit.
...
have IHi i1 j1 : 
    (i1, j1)  \in res1 -> 
    exists i2 j2 : nat, 
    cmove (w lor b) i2 j2 /\ i1 = lsl 1 (of_nat i2 * horizontal + of_nat j2).
...
have /IH' := Hms.
move=> /(_ IHi) [H1 H2 H3 H4 H5]; split => // i1 j1.
- case: ltngtP => // [iLi1|iEi1] _; first by apply: H1.
  move=> cM1.
  by rewrite (cmoveE _ _ _ cM1) -iEi1 -(cmoveE _ _ _ cM) cmE HE.
- case: ltngtP => // [iLi1|iEi1] _; first by apply: H2.
  move=> cM1.
  by rewrite (cmoveE _ _ _ cM1) -iEi1 -(cmoveE _ _ _ cM) cmE1 HE1.
case: ltngtP => // [iLi1|iEi1] _; first by apply: H5.
rewrite -iEi1 => cM1; rewrite (cmoveE _ _ _ cM1) -(cmoveE _ _ _ cM).
...
Lemma fms_moves_corect w b l : 
  ...
  [/\ 
   ...
   (forall i1 j1, cmove (w lor b) i1 j1 ->
      (exists j2, ((lsl 1 (of_nat i1 * horizontal + of_nat j1)), j2) \in l))].
Proof.
move=> Hwf /(fms_moves_rect_corect _ _ _ 0) // /(_ Hwf) [].
- by rewrite addn0 columns_size.
- by move=> j jLc; rewrite addn0 columns_val.
- by move=> i1 j1; rewrite in_nil.
move=> H1 H2 H3 H4 H5; split => // i1 j1; first by apply: H1.
  by apply: H2.
by apply: H5.
Qed.
...
Definition wf w b := [/\ wf_state (w lor b), ~~cwin w & ~~ cwin b].
...
Proof.
move=> [wf_wb ncw_w ncw_b] /eqP /(fms_moves_corect _ _ _ wf_wb) // => []
   [Hww Hwb Hs Hi Hc].
...
Proof.
by move=> [wf_wb ncw_w ncw_b] /eqP /(fms_moves_corect _ _ _ wf_wb) // => [] [].
Qed.
...
Proof.
elim: k r v => //= [] k IH  r v kLw rLwh vLkh.
...
- apply: to_nat_lor_bound.
  ...
  apply: ltn_trans (_ : 2 ^ (nwidth * nhorizontal) < _); last first.
    by rewrite nwB_pow ltn_exp2l.
  rewrite -[nwidth]prednK; last by [].
  rewrite mulSn addnC expnD ltn_mul2r expn_gt0 (leq_trans rLwh) //.
  by rewrite leq_exp2l // leq_mul2r // -subn1 leq_sub.
- rewrite landC; apply: to_nat_land_bound.
  apply: ltn_bit.
    by apply: leq_ltn_trans whLd; rewrite leq_mul2r leq_subLR leq_addl.
  move=> i hwH.
  rewrite full_first_column_spec; case: nltbP => //.
  by rewrite ltnNge (leq_trans _ hwH) // -{1}[to_nat _]mul1n leq_mul2r subn_gt0.
by rewrite to_nat_lsr ltn_divLR ?expn_gt0 // -expnD addnC -mulSn.
Qed.

Lemma get_code_bound  w b t h :
  wf_state (w lor b) -> to_nat (get_code w b t h) < 2 ^ (nwidth * nhorizontal).
Proof.
move=> Hwf.
have Hv : to_nat ((if t then w else b) lor get_border w b) < 
               2 ^ (nwidth * nhorizontal).
  apply: ltn_bit => // i whLi.
  rewrite lor_spec.
  suff -> :  bit (get_border w b) i = false.
    suff : bit (w lor b) i = false.
      by rewrite lor_spec; case: t; do 2 case: bit => //.
    by rewrite (negPf (wf_state_bit_false _ _ _ _)).
  have [/(wf_state_get_border_width _ _ _ Hwf)|//] := boolP (bit _ _).
  by rewrite ltn_divLR // ltnNge whLi.
rewrite /get_code; case: ifP => _; last by exact: Hv.
rewrite /FourInARow.min; case: (_ ?= _).
- by exact: Hv.
- by apply: sym_code_bound.
by exact: Hv.
Qed.

Lemma div_one_lsl_lsr n c : n <? digits -> lsr c n = c / (lsl one n).
Proof.
...
Lemma hput_get_diff w1 w2 b1 b2 t1 t2 wg s h h1 h2 :
  wf_state (w1 lor b1) -> wf_state (w2 lor b2) ->
  length h = nhash ->
   (get_code w1 b1 t1 h1) mod hprime != (get_code w2 b2 t2 h2) mod hprime -> 
  hget w2 b2 t2 (hput w1 b1 t1 wg s h h1) h2 = hget w2 b2 t2 h h2.
Proof.
move=> Hwf1 Hwf2 lH; rewrite /hput /hget.
set c1 := get_code _ _ _ _ mod _ ; set c2:= get_code _ _ _ _ mod _ => c1Dc2.
set u1 := h.[_ <- _].
set u2 := h.[_ <- _].
have c1lE :
    to_nat (2 * lsr c1 lhash) = (to_nat c1 %/ 2 ^ to_nat lhash).*2.
  rewrite to_nat_mul to_nat_lsr mul2n // nwB_pow.
  apply: ltn_trans (_ : 2 ^ (nwidth * nhorizontal).+1 < _); last first.
    by rewrite ltn_exp2l.
  rewrite -mul2n expnS ltn_mul2l // ltn_divLR ?andTb; last first.
    by apply: expn_gt0.
  rewrite -expnD to_nat_mod.
  apply: leq_ltn_trans (leq_mod _ _) _.
  apply: ltn_trans (get_code_bound _ _ _ _ Hwf1) _.
  by rewrite ltn_exp2l.
have c2lE :
    to_nat (2 * lsr c2 lhash) = (to_nat c2 %/ 2 ^ to_nat lhash).*2.
  rewrite to_nat_mul to_nat_lsr mul2n // nwB_pow.
  apply: ltn_trans (_ : 2 ^ (nwidth * nhorizontal).+1 < _); last first.
    by rewrite ltn_exp2l.
  rewrite -mul2n expnS ltn_mul2l // ltn_divLR ?andTb; last first.
  ...
  rewrite -expnD to_nat_mod.
  apply: leq_ltn_trans (leq_mod _ _) _.
  apply: ltn_trans (get_code_bound _ _ _ _ Hwf2) _.
  by rewrite ltn_exp2l.
have mC : 2 * c1 >> lhash = 2 * c2 >> lhash -> c1 >> lhash = c2 >> lhash.
  move=> H.
  apply: to_nat_inj.
  have : to_nat (2 * (lsr c1 lhash)) = to_nat (2 * (lsr c2 lhash)) 
    by rewrite H.
  rewrite [LHS]to_nat_mul ?[RHS]to_nat_mul; last 2 first.
  - rewrite mul2n to_nat_lsr.
    apply: ltn_trans
        (_ : _ < (2 ^ (nwidth * nhorizontal - to_nat lhash).+1)) _; last first.
      by rewrite nwB_pow ltn_exp2l.
    rewrite expnS mul2n ltn_double ltn_divLR ?expn_gt0 //.
  ...
  - rewrite mul2n to_nat_lsr.
    ...
    apply: leq_ltn_trans (leq_mod _ _) _.
    by apply: get_code_bound.
  by move/eqP; rewrite eqn_mul2l => /eqP.
set o := _ || _; have [oT|oF] := ifP o; last first.
  have [cx1mEc2m|cx1mDc2m] := c1 land mhash =P c2 land mhash; last first.
    by rewrite /u2; rewrite [h.[_ <- _].[_]]get_set_other.
  rewrite -cx1mEc2m /u2 [_.[_<-_].[_]]get_set_same; last first.
    rewrite lH; apply/idP/nltbP.
    by rewrite /nhash [X in _ < X]to_nat_lsl_one // landC to_nat_land_bound.
  rewrite [h.[_].[_<-_].[_]]get_set_other; last first.
    move=> H.
    suff : odd (to_nat (2 * c1 >> lhash + 1)).
      by rewrite H c2lE odd_double.
    rewrite to_nat_add; first by rewrite c1lE addn1 oddS odd_double.
    rewrite c1lE addn1.
    apply: ltn_trans 
            (_ : (2 ^ (nwidth * nhorizontal - to_nat lhash)).*2.+1 < _).
      rewrite ltnS ltn_double ltn_divLR // -expnD subnK //.
      rewrite to_nat_mod.
      apply: leq_ltn_trans (leq_mod _ _) _.
      by apply: get_code_bound.
    apply : ltn_trans 
            (_ : 2 ^ (nwidth * nhorizontal - to_nat lhash).+2 < _).
      rewrite 2!expnS !mul2n -[X in _ < X]addnn.
      rewrite -[X in X < _]addn1 ltn_add2l.
      by rewrite -[X in X < _](expn0 2) -mul2n -expnS ltn_exp2l.
    by rewrite nwB_pow ltn_exp2l.
  case: ifP => H.
    rewrite [h.[_].[_<-_].[_]]get_set_other //.
    move=> /add_cancel_r /mC c1lEc2l.
    case/eqP: c1Dc2; apply: to_nat_inj.
    rewrite (int_add_mod c1 (lsl one lhash)) (int_add_mod c2 (lsl one lhash)).
    rewrite -land_power2; last by [].
    rewrite -land_power2; last by [].
    rewrite -div_one_lsl_lsr; last by [].
    rewrite -div_one_lsl_lsr; last by [].
    by rewrite cx1mEc2m c1lEc2l.
  rewrite [h.[_].[_<-_].[_]]get_set_other //.
  move=> /add_cancel_r /mC c1lEc2l.
  case/eqP: c1Dc2; apply: to_nat_inj.
  rewrite (int_add_mod c1 (lsl one lhash)) (int_add_mod c2 (lsl one lhash)).
  ...
  by rewrite cx1mEc2m c1lEc2l.
have [cx1mEc2m|cx1mDc2m] := c1 land mhash =P c2 land mhash; last first.
  by rewrite /u1; rewrite [h.[_ <- _].[_]]get_set_other.
have tc1lSD2tc2l : 2 * c1 >> lhash + 1 != 2 * c2 >> lhash.
  apply/eqP => H.
  suff : odd (to_nat (2 * c1 >> lhash + 1)).
    by rewrite H c2lE odd_double.
  rewrite to_nat_add; first by rewrite c1lE addn1 oddS odd_double.
  ...
  apply : ltn_trans 
    ...
    by rewrite -[X in X < _](expn0 2) -mul2n -expnS ltn_exp2l.
  by rewrite nwB_pow ltn_exp2l.
have tc1lD2tc2lS : 2 * c1 >> lhash != 2 * c2 >> lhash + 1.
  apply/eqP => H.
  suff : odd (to_nat (2 * c2 >> lhash + 1)).
    by rewrite -H c1lE odd_double.
  rewrite to_nat_add; first by rewrite c2lE addn1 oddS odd_double.
  rewrite c2lE addn1.
  apply: ltn_trans 
          (_ : (2 ^ (nwidth * nhorizontal - to_nat lhash)).*2.+1 < _).
    rewrite ltnS ltn_double ltn_divLR // -expnD subnK //.
...
rewrite -cx1mEc2m /u2 [_.[_<-_].[_]]get_set_same; last first.
  rewrite lH; apply/idP/nltbP.
  by rewrite /nhash [X in _ < X]to_nat_lsl_one // landC to_nat_land_bound.
rewrite [h.[_].[_<-_].[_<-_].[_]]get_set_other; last by apply/eqP.
rewrite [h.[_].[_<-_].[_]]get_set_other; last by apply/eqP.
case: ifP => _.
  rewrite [h.[_].[_<-_].[_<-_].[_]]get_set_other; last first.
    by move=> /add_cancel_r /eqP H; case/negP: tc1lDtc2l.
  rewrite [h.[_].[_<-_].[_]]get_set_other; last by apply/eqP.
  by [].
rewrite [h.[_].[_<-_].[_<-_].[_]]get_set_other //; last first.
  by move=> /add_cancel_r /eqP H; case/negP: tc1lDtc2l.
...
Lemma lsr_land_distr a b c : lsr (a land b) c = (lsr a c) land (lsr b c).
Proof.
by apply: bit_ext => i; rewrite !(land_spec, bit_lsr); case: nlebP. 
Qed.

Lemma to_nat_add_le m n : 
  m <=? m + n -> to_nat (m + n) = (to_nat m + to_nat n)%N.
Proof.
...
Lemma lsl_land_decr i j : j <? digits -> (lsl i j) land decr (lsl one j) = 0.
Proof.
move=> jLd; apply: bit_ext => k; rewrite bit_0 land_spec bit_lsl bit_decr //.
by case: nltbP; rewrite ?andbF.
Qed.

Lemma hput_get_same w1 w2 b1 b2 t1 t2 wg s h h1 h2 :
  wf_state (w1 lor b1) -> wf_state (w2 lor b2) -> to_nat s < 2 ^ nscoresize ->
  length h = nhash ->
  (forall i, i <? nhash -> length h.[i] = 2 * (hprime/nhash + 1)) ->
  get_code w1 b1 t1 h1 = get_code w2 b2 t2 h2 -> 
  hget w2 b2 t2 (hput w1 b1 t1 wg s h h1) h2 = s.
Proof.
move=> Hwf1 Hwf2 sLn lH l1H; rewrite /hput /hget => <-.
set c := get_code _ _ _ _ mod _.
have clE :
    to_nat (2 * lsr c lhash) = (to_nat c %/ 2 ^ to_nat lhash).*2.
  rewrite to_nat_mul to_nat_lsr mul2n // nwB_pow.
  apply: ltn_trans (_ : 2 ^ (nwidth * nhorizontal).+1 < _); last first.
    by rewrite ltn_exp2l.
  rewrite -mul2n expnS ltn_mul2l // ltn_divLR ?andTb; last first.
  ...
  rewrite -expnD to_nat_mod.
  apply: leq_ltn_trans (leq_mod _ _) _.
  apply: leq_trans (get_code_bound _ _ _ _ Hwf1) _.
  by rewrite leq_exp2l.
have tclSL2hnS : (2 * c >> lhash + 1 <? 2 * (hprime / nhash + 1)) = true.
  apply/idP/nltbP.
  have -> : to_nat (2 * c >> lhash + 1) = (to_nat c %/ 2 ^ to_nat lhash).*2.+1.
    rewrite to_nat_add; last first.
      rewrite clE addn1.
      apply : ltn_trans 
              (_ : (2 ^ (nwidth * nhorizontal - to_nat lhash)).*2.+1 < _).
        rewrite ltnS ltn_double ltn_divLR ?expn_gt0 // -expnD subnK //.
        rewrite to_nat_mod.
        apply: leq_ltn_trans (leq_mod _ _) _.
        by apply: get_code_bound.
      apply : ltn_trans 
        ...
        by rewrite -[X in X < _](expn0 2) -mul2n -expnS ltn_exp2l.
      by rewrite nwB_pow ltn_exp2l.
    by rewrite clE addn1.
  have -> : to_nat (2 * (hprime / nhash + 1)) = (to_nat hprime %/ 2 ^ to_nat lhash).+1.*2.
    have Hc : to_nat (hprime / nhash + 1) = (to_nat hprime %/ 2 ^ to_nat lhash).+1.
      rewrite to_nat_add; last first.
        ...
        by rewrite to_nat_sub ?ndigitsLwB.
      by rewrite !addn1 to_nat_div to_nat_lsl_one.
  ...
  rewrite doubleS ltnS ltnS leq_double.
  apply: leq_div2r.
  by rewrite to_nat_mod ltnW // ltn_pmod.
have cmLl : c land mhash <? length h.
  apply/nltbP; rewrite lH.
  by rewrite /nhash [X in _ < X]to_nat_lsl_one // landC to_nat_land_bound.
have tclD2lDs : 2 * c >> lhash != 2 * c >> lhash + 1.
  apply/eqP => H.
  suff : odd (to_nat (2 * c >> lhash + 1)).
    by rewrite -H clE odd_double.
  rewrite to_nat_add; first by rewrite clE addn1 oddS odd_double.
  rewrite clE addn1.
  apply: ltn_trans 
          (_ : (2 ^ (nwidth * nhorizontal - to_nat lhash)).*2.+1 < _).
    rewrite ltnS ltn_double ltn_divLR // -expnD subnK //.
  ...
  apply : ltn_trans 
    ...
    by rewrite -[X in X < _](expn0 2) -mul2n -expnS ltn_exp2l.
  by rewrite nwB_pow ltn_exp2l.
set o := _ || _; have [oT|oF] := ifP o.
  rewrite [h.[_<- _].[_]]get_set_same //.
  rewrite [h.[_].[_<- _].[_<- _].[_]]get_set_other; last first.
    by apply/eqP; rewrite eq_sym.
  rewrite [h.[_].[_<- _].[_<- _].[_]]get_set_same; last first.
    rewrite length_set l1H //.
    apply/nltbP.
    by rewrite /nhash [X in _ < X]to_nat_lsl_one // landC to_nat_land_bound.
  rewrite [h.[_].[_<- _].[_]]get_set_same; last first.
    rewrite l1H; last first.
      apply/nltbP.
      by rewrite /nhash [X in _ < X]to_nat_lsl_one // landC to_nat_land_bound.
    apply/nltbP/ltnW.
    have /nltbP := tclSL2hnS.
    rewrite to_nat_add ?addn1 //.
    have [tc_gt0|//|<-] := ltngtP 0 (to_nat (2 * c >> lhash)); last first.
      by rewrite nwB_pow (@ltn_exp2l 2 0).
    apply: leq_ltn_trans (_ : (to_nat (2 * c >> lhash)).*2 < _).
      by rewrite -addn1 -addnn leq_add2l.
    rewrite nwB_pow -[ndigits]prednK; last by [].
    rewrite expnS mul2n ltn_double clE.
    rewrite -[ndigits.-1]prednK; last by [].
    rewrite expnS mul2n ltn_double ltn_divLR; last by rewrite expn_gt0.
    rewrite -expnD to_nat_mod (ltn_trans (ltn_pmod _ _)); first by [].
      by have /nltbP : 0 <? hprime by [].
    have /nltbP : hprime <? lsl one (digits - 2) by [].
    rewrite to_nat_lsl_one; last first.
      by rewrite to_nat_sub // ndigitsLwB.
    move=> /ltn_trans->; first by [].
    by rewrite ltn_exp2l ?expn_gt0.
  rewrite land_lor_distrl.
  have -> : lsl wg locksize land lockmask = 0.
    apply: bit_ext => i; rewrite bit_0 land_spec bit_lsl.
    by rewrite bit_decr; case: nltbP; rewrite ?andbF.
  rewrite lor0.
  suff -> : get_code w1 b1 t1 h1 >> slocksize land lockmask = 
              get_code w1 b1 t1 h1 >> slocksize.
    rewrite eqb_refl lsr_lor.
    have -> : lsr (lsl s scorelocksize) scorelocksize  = s.
      rewrite lsl_lsr_le //.
    ...
    rewrite lsr_add_distl; last first.
      ...
      by rewrite nwB_pow ltn_exp2l.
    rewrite lsr_add lsr_land_distr.
    suff -> : scorelockmask >> (locksize + scoresize) = 0.
      by rewrite land0_r; case: nlebP; rewrite lor0_r.
      rewrite /scorelockmask.
    have -> : locksize + scoresize = scorelocksize by [].
    apply: to_nat_inj; rewrite to_nat_lsr to_nat_sub; last first.
    - by rewrite to_nat_lsl_one; first by rewrite nwB_pow ltn_exp2l.
    - by rewrite to_nat_lsl_one ?(@leq_exp2l 2 0).
    by congr (_ %/ _).
  apply: bit_ext => i.
  rewrite !(land_spec, bit_lsr, bit_decr) //.
  case: nltbP; rewrite !(andbT, andbF) // => /negP; rewrite -leqNgt=> lLi.
  case: nlebP => // iLsi.
  apply/sym_equal/(bit_false_lt _ _ _ _ (get_code_bound _ _ _ _ Hwf1)).
  rewrite to_nat_add_le.
    by apply: leq_trans (_ :  to_nat slocksize + to_nat locksize <= _).
  apply/nlebP; rewrite to_nat_add ?leq_addl //.
  rewrite -addnC -to_nat_add_le; first by rewrite to_nat_bounded.
  by rewrite add_comm; apply/nlebP.
rewrite [h.[_<- _].[_]]get_set_same //.
rewrite [h.[_].[_<- _].[_]]get_set_other; last by apply/eqP; rewrite eq_sym.
rewrite [h.[_].[_<- _].[_]]get_set_same //; last first.
  rewrite l1H //; last first.
  apply/nltbP.
  by rewrite /nhash [X in _ < X]to_nat_lsl_one // landC to_nat_land_bound.
rewrite ifF; last by move: oF; rewrite /o; case: (_ =? _).
  rewrite land_lor_distrl lsl_land_decr // lor0.
have -> : get_code w1 b1 t1 h1 >> slocksize land lockmask =
          get_code w1 b1 t1 h1 >> slocksize.
  apply: bit_ext => i.
  rewrite land_spec bit_lsr; case: nlebP => // iLsi.
  rewrite bit_decr //; case: nltbP; rewrite (andbT, andbF) //.
  move=> /negP; rewrite -leqNgt => lLi; apply/sym_equal.
  apply: bit_false_lt (get_code_bound _ _ _ _ Hwf1).
  by rewrite add_comm to_nat_add_le 1?addnC //; rewrite add_comm; apply/nlebP.
  case: neqbP => // _.
apply: bit_ext => i.
rewrite !(land_spec, bit_decr, bit_lsr, bit_lsl, lor_spec) //.
case: nltbP => [liLl|/negP].
  rewrite ifF; last first.
    apply/idP => iLli; move: liLl.
    by rewrite ltnNge add_comm to_nat_add_le ?leq_addl // add_comm.
  apply/sym_equal/(bit_false_lt _ _ _ _ sLn).
  rewrite leqNgt; apply/negP=> iLn.
  move: liLl; rewrite to_nat_add //.
  apply: ltn_trans (_ : to_nat locksize + nscoresize < _).
    by rewrite ltn_add2l.
  apply: ltn_trans (_ : 2 ^ 49 < _); first by [].
  by rewrite nwB_pow ltn_exp2l.
rewrite -leqNgt => lLli.
have liE : to_nat (locksize + i) = (to_nat locksize + to_nat i)%N.
  by rewrite to_nat_add_le //; apply/nlebP.
rewrite ifT /=; last by apply/nlebP; rewrite liE leq_addl.
case: (nltbP i scoresize) => [iLs /=|/negP]; last first.
  rewrite andbF -leqNgt => sLi.
  by apply/sym_equal/(bit_false_lt _ _ _ sLi).
rewrite ifF; last first.
  case: nlebP => //; rewrite leqNgt => /negP[].
  rewrite liE.
  apply: ltn_trans (isT : to_nat locksize + to_nat scoresize < to_nat digits).
  by rewrite ltn_add2l.
rewrite add_comm addK ifT /=; last by move/nltbP : iLs ->.
rewrite ifT; last first.
  apply/nlebP.
  rewrite add_comm liE to_nat_add liE; first by rewrite leq_addl.
  rewrite addnA (ltn_trans (_ : _ < to_nat slocksize + 
                                  to_nat locksize + to_nat scoresize)) //.
  apply: ltn_trans (_ : _ < 53) _; first by [].
  by rewrite (ltn_trans (_ : _ < 2 ^ 6)) // nwB_pow ltn_exp2l.
rewrite (bit_false_lt _ _ _ _ (get_code_bound _ _ _ _ Hwf1)) //.
  by rewrite orbF andbT.
rewrite to_nat_add add_comm liE addnA; first by rewrite leq_addr.
rewrite (ltn_trans (_ : _ < to_nat slocksize + 
                                to_nat locksize + to_nat scoresize)) //.
apply: ltn_trans (_ : _ < 53) _; first by [].
by rewrite (ltn_trans (_ : _ < 2 ^ 6)) // nwB_pow ltn_exp2l.
Qed.
...
Definition hash_table_evalt 
         (wstate bstate : int) (turn : bool) 
         (hash_table : array (array int)) height := 
   ...
   let s2 := (val2 >> locksize) land scoremask in
    (((val1 land lockmask) != lock) ||
     (down_score s1 <= evalt wstate bstate turn <= up_score s1)%N) && 
    (((val2 land lockmask) != lock) ||
     (down_score s2 <= evalt wstate bstate turn <= up_score s2)%N).

Lemma hash_table_prop_evalt w b t h hg :
  hash_table_evalt w b t h hg -> 
  (down_score (hget w b t h hg) <= evalt w b t <= up_score (hget w b t h hg))%N.
Proof.
rewrite /hash_table_evalt /hget !neq_eqE.
case: eqP => _ /=; case: eqP => _ //=.
- by case/andP.
- by case/andP.
move=> _; case: t => /=; first by case/or3P: (evalOr w b) => /eqP->.
...
Lemma wf_state_sym_code w b : 
  wf_state (w lor b) -> 
  wf_state (sym_code nwidth zero w lor sym_code nwidth zero b).
Proof.
move=> Hwf; apply/andP; split.
...
suff -> : get_column (sym_code nwidth zero w lor sym_code nwidth zero b) i =
    ...
    have /andP[_ /forallP/(_ (width - (i + 1)))/implyP] := Hwf.
    apply; apply/nltbP.
...
Lemma transpose_sget w1 w2 b1 b2 : 
  ...
  transpose w1 w2 && transpose b1 b2.
move=> Hwf1 Hwf2 w1b1E0 w2b2E0 Hf.
pose w3 := sym_code nwidth zero w1.
pose b3 := sym_code nwidth zero b1.
have Hwf3 : wf_state (w3 lor b3) by apply: wf_state_sym_code.
have Ha3 : w3 land b3 = 0.
  ...
  have [kLwh|whLk] := ltnP (to_nat k) (nwidth * nhorizontal); last first.
    ...
    by apply/idP/negP; apply: wf_state_bit_false whLk.
  rewrite bit_cell cell_land.
  have khLw : to_nat k %/ nhorizontal < nwidth by rewrite ltn_divLR.
  have khLh : to_nat k %% nhorizontal < nhorizontal by apply: ltn_pmod.
...
have [kLwh|whLk] := ltnP (to_nat k) (nwidth * nhorizontal); last first.
  have -> : bit (sget_code w2 b2) k = false.
    ...
    have -> : bit (get_border w2 b2) k = false.
      apply/idP => HH.
      have := wf_state_get_border_width _ _ _ Hwf2 HH.
      by rewrite ltn_divLR // ltnNge whLk.
    rewrite orFb.
  ...
  have -> : bit (get_border w3 b3) k = false.
    apply/idP => HH.
    have := wf_state_get_border_width _ _ _ Hwf3 HH.
    by rewrite ltn_divLR // ltnNge whLk.
  rewrite orFb.
...
Proof.
move=> Hwf1 Hwf2 Hw1ab1 Hw2ab2 Ht1 Ht2.
apply: bit_ext => k.
have [kLwh|whLk] := ltnP (to_nat k) (nwidth * nhorizontal); last first.
  rewrite lor_spec.
  have -> : bit (get_border w1 b1) k = false.
    apply/idP => HH.
    have := wf_state_get_border_width _ _ _ Hwf1 HH.
    by rewrite ltn_divLR // ltnNge whLk.
  rewrite lor_spec.
  have -> : bit (get_border w2 b2) k = false.
    apply/idP => HH.
...
Lemma evalt_get_code w1 w2 b1 b2 t1 t2 h1 h2 : 
  wf_state (w1 lor b1) -> wf_state (w2 lor b2) ->
  w1 land b1 = 0 -> w2 land b2 = 0 ->
  get_code w1 b1 t1 h1 = get_code w2 b2 t2 h2 -> 
  evalt w1 b1 t1 = evalt w2 b2 t2.
Proof.
move=> Hwf1 Hwf2 Hw1ab1 Hw2ab2.
have [->|->] : 
    t1 = t2 \/ t1 = ~~ t2 by case: t1; case: t2; (try by left); right.
  have := get_code_correct w1 b1 t2 h1.
  have := get_code_correct w2 b2 t2 h2.
  case: t2.
    move=> [-> [->|Ht1]|Ht1 [->|Ht2]].
    ...
    have : sget_code w1 b1 = sget_code w2 b2.
      ...
      by rewrite -HH.
    by move=> /sget_code_uniq[] // -> ->.
  rewrite lorC in Hwf1.
  rewrite lorC in Hwf2.
  rewrite landC in Hw1ab1.
  rewrite landC in Hw2ab2.
  move=> [-> [->|Ht1]|Ht1 [->|Ht2]].
  - by move=> /sget_code_uniq [] // <- <-.
  - move=> gcE; rewrite gcE in Ht1.
    apply/sym_equal.
  ...
  - move=> gcE; rewrite -gcE in Ht1.
    ...
    by apply: eval_transpose.
  move=> HH.
  have : sget_code b1 w1 = sget_code b2 w2.
    apply: transpose_transpose_sget_code_eq => //.
    - by rewrite landC.
    - by apply: Ht2.
    by rewrite HH.
  by move=> /sget_code_uniq[] // -> ->.
...
case: t2.
  ...
  - move=> gcE; rewrite -gcE in Ht1.
    ...
    by apply: eval_transpose.
  move=> HH.
  have : sget_code b1 w1 = sget_code w2 b2.
    apply: transpose_transpose_sget_code_eq => //.
    - by rewrite landC.
    - by apply: Ht2.
    by rewrite HH.
...
- move=> gcE; rewrite -gcE in Ht1.
  ...
  by apply: eval_transpose.
move=> HH.
have : sget_code w1 b1 = sget_code b2 w2.
  apply: transpose_transpose_sget_code_eq => //.
  - by rewrite landC.
  - by apply: Ht2.
  by rewrite HH.
...
Lemma has_table_evalt_hput_get w1 b1 t1 wg1 s h hg1 :
  wf_state (w1 lor b1) -> w1 land b1 = 0 ->
  (down_score s <= evalt w1 b1 t1 <= up_score s)%N ->
  length h = nhash ->
  (forall i, i <? nhash -> length h.[i] = 2 * (hprime/nhash + 1)) ->
  (forall w2 b2 t2 hg2, wf_state (w2 lor b2) -> w2 land b2 = 0 -> 
     hash_table_evalt w2 b2 t2 h hg2) -> 
  (forall w2 b2 t2 hg2, wf_state (w2 lor b2) -> w2 land b2 = 0 ->
     hash_table_evalt w2 b2 t2 (hput w1 b1 t1 wg1 s h hg1) hg2).
Proof.
move=> Hwf1 Ha1 w1b1P lEh liEh Hh w2 b2 t2 hg2 Hwf2 Ha2.
have sLs : to_nat s < 2 ^ nscoresize by apply: evalt_score_bound w1b1P.
rewrite /hash_table_evalt /hput /hget.
set c1 := get_code _ _ _ _ mod _; set c2 := get_code _ _ _ _ mod _.
have clE c :
...
have clSE c :
    to_nat c < 2 ^ (nwidth * nhorizontal) ->
    to_nat (2 * lsr (c mod hprime) lhash + 1) = 
    (to_nat (c mod hprime) %/ 2 ^ to_nat lhash).*2.+1.
  move=> cLwh.
  rewrite to_nat_add; first by rewrite addn1 clE.
  rewrite addn1 clE; last by [].
...
have tc1lDtc2lDs : 2 * c1 >> lhash != 2 * c2 >> lhash + 1.
  apply/eqP => H.
  suff : odd (to_nat (2 * c2 >> lhash + 1)).
    rewrite -H clE; last by apply: get_code_bound.
    by rewrite odd_double.
  rewrite to_nat_add.
    rewrite clE; last by apply: get_code_bound.
    by rewrite addn1 oddS odd_double.
  rewrite clE ?addn1; last by apply: get_code_bound.
  apply: ltn_trans 
          (_ : (2 ^ (nwidth * nhorizontal - to_nat lhash)).*2.+1 < _).
    rewrite ltnS ltn_double ltn_divLR // -expnD subnK //.
    rewrite to_nat_mod.
...
have tc2lDtc1lDs : 2 * c2 >> lhash != 2 * c1 >> lhash + 1.
  ...
  apply : ltn_trans 
    ...
    by rewrite -[X in X < _](expn0 2) -mul2n -expnS ltn_exp2l.
  by rewrite nwB_pow ltn_exp2l.
have phE : to_nat (2 * (hprime / nhash + 1)) = (to_nat hprime %/ 2 ^ to_nat lhash).+1.*2.
  have Hc : to_nat (hprime / nhash + 1) = (to_nat hprime %/ 2 ^ to_nat lhash).+1.
    rewrite to_nat_add; last first.
      apply: ltn_trans (_ : (to_nat (hprime / nhash)).*2 < _).
        rewrite -addnn ltn_add2l.
...
have [oT|oF] := ifP o.
  have [cx1mEc2m|cx1mDc2m] := (c1 land mhash) =P (c2 land mhash); last first.
    ...
    by have := Hh w2 b2 t2 hg2 Hwf2 Ha2.
  rewrite cx1mEc2m.
  rewrite [h.[_ <- _].[_]]get_set_same; last by rewrite lEh; apply/nltbP.
  set u1 := h.[_].[_ <- _].
  rewrite [u1.[_ <- _].[_]]get_set_other; last by apply/eqP; rewrite eq_sym.
  have [cx1lEc2l|cx1lDc2l] := (lsr c1 lhash) =P (lsr c2 lhash); last first.
    have tc1lE : to_nat (2 * c1 >> lhash) = (to_nat (c1 >> lhash)).*2.
      by rewrite clE; [rewrite to_nat_lsr | apply: get_code_bound].
    have tc2lE : to_nat (2 * c2 >> lhash) = (to_nat (c2 >> lhash)).*2.
      by rewrite clE; [rewrite to_nat_lsr | apply: get_code_bound].
    have tc1lDtc2l : 2 * c1 >> lhash <> 2 * c2 >> lhash.
      contradict cx1lDc2l.
      apply/to_nat_inj/double_inj.
      rewrite -!mul2n -[2%N]/(to_nat 2) -to_nat_mul ?cx1lDc2l.
    ...
    have tc2lSDtc2lS : 2 * c1 >> lhash + 1 <> 2 * c2 >> lhash + 1.
      by contradict tc1lDtc2l; rewrite -[LHS](addK _ 1) tc1lDtc2l addK.
    rewrite /u1 [_.[_ <- _].[2 * (lsr c2 lhash)]]get_set_other; last by [].
    rewrite [_.[_ <- _].[_ + 1]]get_set_other; last by [].
    rewrite [_.[_ <- _].[_ + 1]]get_set_other; last by apply/eqP.
    by have := Hh w2 b2 t2 hg2 Hwf2 Ha2.
  rewrite /u1 cx1lEc2l.
  rewrite [_.[_<-_].[2 * c2 >> lhash]]get_set_same; last first.
    apply/nltbP; rewrite clE; last by apply: get_code_bound.
    rewrite liEh; last first.
      apply/nltbP.
      rewrite [X in _ < X]to_nat_lsl_one; last by [].
      rewrite landC; apply: to_nat_land_bound.
      rewrite to_nat_decr; last by [].
      by rewrite [X in X.-1 < _]to_nat_lsl_one ?prednK.
    rewrite phE ltn_double ltnS.
    apply: leq_div2r.
  ...
  rewrite [_.[_<-_].[2 * c2 >> lhash + 1]]get_set_same; last first.
    ...
    rewrite liEh; last by apply/nltbP.
    apply/nltbP; rewrite phE.
    rewrite clSE; last by apply: get_code_bound.
  ...
  rewrite lor0.
  have -> : get_code w1 b1 t1 hg1 >> slocksize land lockmask = 
            get_code w1 b1 t1 hg1 >> slocksize.
  ...
  apply/andP; split.
    case: eqP => [c1Ec2|//].
    rewrite orFb.
    suff -> : evalt w2 b2 t2 = evalt w1 b1 t1 by [].
    suff : get_code w2 b2 t2 hg2 = get_code w1 b1 t1 hg1 
      by apply: evalt_get_code.
    apply: to_nat_inj.
    set x1 := to_nat (get_code w1 _ _ _).
    set x2 := to_nat (get_code w2 _ _ _).
    pose m1 := 2 ^ to_nat slocksize.
    have x1m1Ex2m1 : x1 %/ m1 = x2 %/ m1.
      have : to_nat (get_code w1 b1 t1 hg1 >> slocksize) = 
             to_nat (get_code w2 b2 t2 hg2 >> slocksize).
        by congr (to_nat _).
    ...
    have m1Lhp :  m1 < to_nat hprime.
       have /nltbP : (lsl one slocksize) <? hprime by [].
       by rewrite to_nat_lsl_one.
    ...
    Search ((_ + _)%% _).
       ...
       to_nat_mod to_nat_mod to_nat_mod.
       
      Search (_ land _) (_ mod _).
  ...
  have /andP[_ ->] := Hh w2 b2 t2 hg2 Hwf2.
    apply: bit_ext => i.
    byrewrite land_spec bit_decr.
    rewrite /locksize /scoresize.
  Search (lsl _ (_ + _)).
  set uu := _ || _.
  set uu1 := _ lor _.
  Search ((_ lor _) land _).
  rewrite lor_land_distr.

    Search (to_nat _) (_ <= _) (_ + _).
    rewrite -addnC -to_nat_add_le; first by rewrite to_nat_bounded.
    by rewrite add_comm; apply/nlebP.
 
    apply/sym_equal/bit_false_lt.   

  rewrite /slocksize /lockmask.

  set v1 := _ land scoremask; have -> : v1 = h.[c2 land mhash].[2 * c2 >> lhash + 1].
    admit.
  set v2 := _ land lockmask; have -> : v2 = get_code w1 b1 t1 hg1 >> slocksize.
    admit.
  let val2 := (ht.[key + 1]) in
   let s1 := (val2 >> scorelocksize) in 
   let s2 := (val2 >> locksize) land scoremask in
  ...
  have := Hh w2 b2 t2 hg2 Hw
  f2.
  rewrite /hash_table_evalt.
  ...
  set h2 := h1.[_].
  set 
  have -> : lsr (lsl s (scorelocksize lor (h2 land scorelockmask))) scorelocksize = s.
  set h1 := h.[_].
  set h2 := h1.[_].
  set h4 := h2.[_ <- _].
  set h5 := h4.[_].
  ...
  rewrite [_.[_<-_].[2 * c2 >> lhash]]get_set_same; last first.
     admit.
      ...
      r
      rewrite -cx1lEc2l [u1.[_ <- _].[_]]get_set_same; last first.
        admit.
      rewrite /u1 [h.[_].[_<-_].[_]]get_set_same; last first.
        admit.
      rewrite /o /hash_table_evalt -/c2 /u1 cx1lEc2l cx1mEc2m .
      set v1 := lsr _ scorelocksize.
      have -> : v1 = s by admit.
      set v2 := _ land scoremask.
      have -> : v2 = s by admit.
      set r1 := _ land lockmask.
      have -> : r1 = get_code w1 b1 t1 hg1 >> slocksize by admit.
      
      have := Hh w2 b2 t2 hg2 Hwf2.
      rewrite /hash_table_evalt -/c2.
      rewrite w1b1P.
      
      case: eqP => /=.
      have := Hu w2 b2 t2 hg2 Hwf2.
      rewrite /hash_table_prop -/c2.
    .
...
have := 
  ...
  wf_state (w2 lor b2) -> to_nat s < 2 ^ nscoresize ->
  length h = nhash ->
  (forall i, i <? nhash -> length h.[i] = 2 * (hprime/nhash + 1)) ->
  get_code w1 b1 t1 h1 = get_code w2 b2 t2 h2 -> 
  hget w2 b2 t2 (hput w1 b1 t1 wg s h h1) h2 = s.
