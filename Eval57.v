...
Require Import FourInARow57.

Set Implicit Arguments.
Unset Strict Implicit.
Unset Printing Implicit Defensive.

Definition htop_eval57 w b ht :=
  let: PRes sc _ _ := 
    alpha_beta (1 + nheight * nwidth) 0 w b loss win zero ht in sc.

Lemma htop_eval57_equiv w b ht : 
  htop_eval 5 7 w b ht  = htop_eval57 w b ht.
Proof. by []. Qed.

Lemma htopeval57_correct w b ht : 
 valid_posb 5 7 w b -> valid_htable 5 7 ht -> valid_eval 5 7 w b (htop_eval57 w b ht).
Proof.
move=> Hv Ht.
rewrite -htop_eval57_equiv.
apply: htopeval_correct; [done|done|done|done| |done|done].
rewrite -to_nat_lsl_one; last by [].
by apply: nltbP.
Qed.

Definition top_eval57 w b := htop_eval57 w b (make_hash tt) .

Lemma topeval57_correct w b : valid_posb 5 7 w b -> valid_eval 5 7 w b (top_eval57 w b).
Proof.
move=> Hv.
rewrite -[top_eval57 _ _]htop_eval57_equiv.
