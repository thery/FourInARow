...
Require Import FourInARow65.

Set Implicit Arguments.
Unset Strict Implicit.
Unset Printing Implicit Defensive.

Definition htop_eval65 w b ht :=
  let: PRes sc _ _ := 
    alpha_beta (1 + nheight * nwidth) 0 w b loss win zero ht in sc.

Lemma htop_eval65_equiv w b ht : 
  htop_eval 6 5 w b ht  = htop_eval65 w b ht.
Proof. by []. Qed.

Lemma htopeval65_correct w b ht : 
 valid_posb 6 5 w b -> valid_htable 6 5 ht -> valid_eval 6 5 w b (htop_eval65 w b ht).
Proof.
move=> Hv Ht.
rewrite -htop_eval65_equiv.
apply: htopeval_correct; [done|done|done|done| |done|done].
rewrite -to_nat_lsl_one; last by [].
by apply: nltbP.
Qed.

Definition top_eval65 w b := htop_eval65 w b (make_hash tt) .

Lemma topeval65_correct w b : valid_posb 6 5 w b -> valid_eval 6 5 w b (top_eval65 w b).
Proof.
move=> Hv.
rewrite -[top_eval65 _ _]htop_eval65_equiv.
